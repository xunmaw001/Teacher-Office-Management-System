...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>身份证号</label>
										<input name="shenfenzhenghao" id="shenfenzhenghao" name="shenfenzhenghao" class="form-control" placeholder="身份证号">
									</div>
									<div class="form-group col-md-6">
										<label>学院</label>
									...
									<div class="form-group col-md-6">
										<label>系所</label>
										<select id="xisuoSelect" name="xisuo" class="form-control">
										</select>	
									</div>	
	...
	<script>
		...
		function xisuoSelect(){
			http("option/xisuo/xisuo","GET",{},(res)=>{
				if(res.code == 0){
					xisuoOptions = res.data;
					var nullOption = document.createElement('option');
					var xisuoSelect = document.getElementById('xisuoSelect');
					xisuoSelect.appendChild(nullOption);
					for(var i=0;i<xisuoOptions.length;i++){
						var xisuoOption = document.createElement('option');
						xisuoOption.setAttribute('name','xisuoOption');
						xisuoOption.setAttribute('value',xisuoOptions[i]);
						xisuoOption.innerHTML = xisuoOptions[i];									
						if(ruleForm.xisuo == xisuoOptions[i]){
							xisuoOption.setAttribute('selected','selected');
						}
						xisuoSelect.appendChild(xisuoOption);
					}
				}
			});
		}	
		...
		function submit() {
                //获取表单数据前取消禁用
                $("#xueyuanSelect").attr('disabled',false);
                //获取表单数据前取消禁用
                $("#xisuoSelect").attr('disabled',false);
		...
		function dataBind(){
			...
			var xisuoOptions = document.getElementsByClassName("xisuoOption");
			for(var xisuoCount = 0; xisuoCount < xisuoOptions.length;xisuoCount++){
				if(xisuoOptions[xisuoCount].getAttribute('value') == ruleForm.xisuo){
					xisuoOptions[xisuoCount].setAttribute('selected','selected');
				}
			}
		}		
		//图片显示
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.xisuo != null && res.data.xisuo != ''){
						...
						var xisuoOptions = document.getElementsByClassName("xisuoOption");
						for(var xisuoCount = 0; xisuoCount < xisuoOptions.length;xisuoCount++){
							if(xisuoOptions[xisuoCount].getAttribute('value') == res.data.xisuo){
								xisuoOptions[xisuoCount].setAttribute('selected','selected');
							}
						}
					}
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#xueyuanSelect").attr('disabled',true);
                $("#xisuoSelect").attr('disabled',true);
            }
		}
